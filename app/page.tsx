...
export default function HomePage() {
  ...
  const filteredEvents = useMemo(() => {
    return events.filter((ev) => {
      ...
      const matchesCats =
          selectedCats.length === 0 || selectedCats.some((c) => ev.categories.includes(c));

      const evStart = new Date(ev.startDate).getTime();
      const matchesDateStart = !dateRange.start || evStart >= new Date(dateRange.start).getTime();
      const matchesDateEnd = !dateRange.end || evStart <= new Date(dateRange.end).getTime();
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
        <div className="text-center mb-12">
          <h1 className="text-4xl font-extrabold text-slate-900 mb-4 tracking-tight">
            Descubre <span className="text-indigo-600">eventos comunitarios</span> en nuestro campus
          </h1>
          <p className="text-lg text-slate-600 max-w-2xl mx-auto">
            Desde charlas hasta concursos, descubre los mejores eventos y actividades organizados por los estudiantes.
          </p>
        </div>
        ...
        {filteredEvents.length > 0 ? (
            ...
            <div className="text-center py-20 bg-white rounded-3xl border border-dashed border-slate-300">
              ...
              <h3 className="text-xl font-semibold text-slate-800">Sin eventos encontrados</h3>
              <p className="text-slate-500">Intenta ajustar tus filtros o términos de búsqueda.</p>
            </div>
        )}
      </div>
  );
