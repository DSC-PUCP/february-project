...
import { getEventsByOrgId, deleteEvent, getAllEvents } from '@/lib/actions/events';
import { getAllOrganizations, createOrganization, deleteOrganization } from '@/lib/actions/organizations';
...
export default function DashboardPage() {
    ...
    const handleCreateOrg = async (e: React.FormEvent) => {
        ...
        try {
            ...
            setCreateOrgError((err as Error).message || 'Error al crear la organización.');
        } finally {
    ...
    return (
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
                ...
                {isAdmin ? (
                    <>
                        <div className="bg-white p-6 rounded-2xl border border-slate-200 shadow-sm">
                            <div className="text-2xl font-bold text-indigo-600 mb-2">{orgs.length}</div>
                            <div className="text-slate-600">Organizaciones</div>
                        </div>
                        <div className="bg-white p-6 rounded-2xl border border-slate-200 shadow-sm">
                            <div className="text-2xl font-bold text-green-600 mb-2">{allEvents.filter(e => new Date(e.startDate) >= now).length}</div>
                            <div className="text-slate-600">Eventos próximos</div>
                        </div>
                        <div className="bg-white p-6 rounded-2xl border border-slate-200 shadow-sm">
                            <div className="text-2xl font-bold text-slate-600 mb-2">{allEvents.length}</div>
                ...
                {isAdmin ? (
                    <>
                        <div className="flex items-center justify-between mb-6">
                            ...
                            <button
                                onClick={() => { setCreateOrgModal(true); setTempPassword(''); setCreateOrgError(''); }}
                        ...
                        {orgsLoading ? (
                            <div className="text-slate-500 text-center py-8">Cargando organizaciones...</div>
                        ) : orgs.length === 0 ? (
                            <div className="text-center py-10 text-slate-500">No hay organizaciones registradas.</div>
                        ) : (
                            <div className="space-y-3">
                                {orgs.map((org) => (
                                    <div key={org.id} className="flex items-center gap-4 p-4 rounded-xl border border-slate-100 hover:border-slate-200 transition-colors">
                                        ...
                                        <div className="flex-1 min-w-0">
                                            <p className="font-bold text-slate-900 truncate">{org.name ?? org.email}</p>
                        ...
                        {tempPassword ? (
                            <>
                                <div className="text-center mb-6">
                                    ...
                                    <h2 className="text-xl font-bold text-slate-900 mb-2">Organización creada</h2>
                                    <p className="text-slate-600 text-sm">Comparte esta contraseña temporal con la organización:</p>
                                </div>
                                <div className="bg-slate-50 border border-slate-200 rounded-xl p-4 text-center mb-6">
                                    ...
                                    <p className="text-2xl font-mono font-bold text-indigo-600 tracking-widest">{tempPassword}</p>
                                </div>
                                <p className="text-xs text-slate-500 text-center mb-6">Al iniciar sesión por primera vez, el sistema les pedirá cambiarla.</p>
                                <button
                                    onClick={() => { setCreateOrgModal(false); setTempPassword(''); }}
                            ...
                            <form onSubmit={handleCreateOrg}>
                                <h2 className="text-xl font-bold text-slate-900 mb-6">Nueva organización</h2>
                                <div className="mb-5">
                                    <label className="block text-sm font-semibold text-slate-700 mb-2">Email de la organización</label>
                                    <input
                                        type="email"
                                        required
                                        value={newOrgEmail}
                                    ...
                                    />
                                    <p className="text-xs text-slate-500 mt-1">Se generará una contraseña temporal automáticamente.</p>
                                </div>
                                {createOrgError && (
                                    <div className="mb-4 p-3 bg-red-50 border border-red-200 text-red-700 rounded-xl text-sm">{createOrgError}</div>
                                )}
                                <div className="flex gap-3">
                                    <button
                                        type="button"
                                        onClick={() => { setCreateOrgModal(false); setCreateOrgError(''); }}
                                        className="flex-1 py-3 rounded-xl border border-slate-200 text-slate-700 font-bold hover:bg-slate-50 transition-colors"
                                    >
                                        Cancelar
                                    </button>
            ...
            {deleteOrgModal && (
                <div className="fixed inset-0 bg-slate-900/50 backdrop-blur-sm z-[110] flex items-center justify-center p-4">
                    <div className="bg-white rounded-3xl w-full max-w-md shadow-2xl p-8">
                        <h2 className="text-xl font-bold text-slate-900 mb-3">¿Eliminar organización?</h2>
                        <p className="text-slate-600 mb-2">Esta acción no se puede deshacer. Se eliminará:</p>
                        <p className="font-semibold text-slate-800 mb-6">{deleteOrgModal.name ?? deleteOrgModal.email}</p>
                        <div className="flex gap-3">
                            <button
