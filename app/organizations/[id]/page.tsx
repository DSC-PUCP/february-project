        ...
        <div className="max-w-4xl mx-auto px-4 py-10">
            ...
            <div className="bg-white rounded-3xl shadow-sm border border-slate-200 overflow-hidden mb-8">
                ...
                <div className="px-8 pb-8">
                    ...
                    {org.description && (
                        <div className="mb-6">
                            <h3 className="text-xs font-bold text-slate-400 uppercase tracking-widest mb-2">Acerca de</h3>
                    ...
                    {Array.isArray(org.contacts) && org.contacts.length > 0 && (
                        <div>
                            ...
                            <div className="flex flex-wrap gap-3">
                                {(org.contacts as { type: string; value: string }[]).map((contact, idx) => (
                                    <a
                                        key={idx}
                                        href={
                                            contact.type === 'link'
                                    ...
                                    >
                                        ...
                                        {contact.type === 'whatsapp' ? 'WhatsApp' : contact.type === 'email' ? 'Email' : 'Sitio web'}
                                    </a>
                                ))}
                            </div>
                        </div>
            ...
            <div className="bg-white rounded-3xl border border-slate-200 shadow-sm p-8">
                <h2 className="text-xl font-bold text-slate-900 mb-6">Eventos de la organización</h2>
                ...
                {events.length === 0 ? (
                    ...
                    <div className="space-y-4">
                        ...
                        {upcomingEvents.length > 0 && (
                            <div>
                                <h3 className="text-xs font-bold text-slate-400 uppercase tracking-widest mb-3">Próximos</h3>
                        ...
                        {pastEvents.length > 0 && (
                            <div className="mt-6">
                                <h3 className="text-xs font-bold text-slate-400 uppercase tracking-widest mb-3">Realizados</h3>
                                <div className="space-y-3">
                                    {pastEvents.map((ev) => (
                                        <EventRow key={ev.id} event={ev} categories={categories} isPast />
                                    ))}
                                </div>
                            </div>
                        )}
...
function EventRow({ event, categories, isPast = false }: { event: Event; categories: Category[]; isPast?: boolean }) {
    ...
    return (
        ...
        >
            ...
            <div className="flex-1 min-w-0">
                <div className="flex items-center gap-2 mb-1">
                    ...
                    {eventCats.slice(0, 2).map((c) => (
                        <span key={c.id} className="text-xs font-semibold text-indigo-600 bg-indigo-50 px-2 py-0.5 rounded-full">{c.name}</span>
                    ))}
                </div>
            ...
            <svg className="w-5 h-5 text-slate-300 group-hover:text-indigo-400 shrink-0 transition-colors" fill="none" stroke="currentColor" viewBox="0 0 24 24">
